...
const CreateFunnelPage: React.FC<CreateFunnelPageProps> = ({
    ...
    const onSubmit = async(
        values: z.infer<typeof FunnelPageSchema>
    )=>{
        ...
        try {
            ...
            toast({
                variant: 'destructive',
                title: 'Oppse!',
                description: 'It Does Not Save Funnel Page Details'
            })
        }
    }
  return ( 
  <Card className='w-full'>
    <CardHeader>
        <CardTitle>Funnel Page Details</CardTitle>
        <CardDescription>
          Funnel pages flow in the order they are created in, by default. You
    ...
    <CardContent>
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col gap-6'>
                <FormField disabled={form.formState.isSubmitting} control={form.control} name='name' render={({field})=>(
                ...
                <FormField disabled={form.formState.isSubmitting || order === 0} control={form.control} name='pathName' render={({field})=>(
                    <FormItem className='flex-1'>
                        <FormLabel>Path Name</FormLabel>
                        <FormControl>
                            <Input placeholder='Path Name for the Page' {...field} value={field.value?.toLowerCase()}/>
                        </FormControl>
                        <FormMessage/>
                    </FormItem>
                )}/>
                <div className='flex items-center gap-2'>
                    ...
                    {defaultData?.id && (
                        <Button variant={'outline'} className='w-22 self-end border-destructive text-destructive hover:bg-destructive' disabled={form.formState.isSubmitting} type='button' onClick={async()=>{
                    ...
                    {defaultData?.id && (
                        <Button variant={'outline'} size={'icon'} disabled={form.formState.isSubmitting} type='button' onClick={async()=>{
                            const response = await getFunnels(defaultData.id)
