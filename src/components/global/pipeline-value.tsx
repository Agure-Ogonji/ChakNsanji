...
const PipelineValue = ({subaccountId}: Props) => {
    ...
    const [pipelineCloseValue, setPipelineCloseValue] = useState(0)
    ...
    const totalPipelineValue = useMemo(()=>{
        if(pipelines.length){
            return(
                pipelines.find((pipeline)=>pipeline.id === selectedPipelineId)?.Lane?.reduce((totalLanes, lane, currentLaneIndex, array)=>{
    ...
    const pipelineRate = useMemo(
        () => (pipelineCloseValue / (totalPipelineValue + pipelineCloseValue)) * 100,
        [pipelineCloseValue, totalPipelineValue]
    )
  return (
            ...
            <div className='flex items-center justify-between'>
                <div>
                    <p className='text-xs text-muted-foreground'>
                        Closed ${pipelineCloseValue}
                    </p>
                </div>
                <div>
                    <p className='text-xs text-muted-foreground'>
                        Total ${totalPipelineValue + pipelineCloseValue}
                    </p>
                </div>
            </div>
