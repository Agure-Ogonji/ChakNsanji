...
import { saveActivityLogsNotification, updateAgencyDetails, updateFunnelProducts } from '@/lib/queries'
...
const FunnelProductsTable: React.FC<FunnelProductsTableProps> = ({
  ...
  return (
    <>
        ...
        <TableBody className='font-medium truncate'>
          {products.map((product)=>(
            <TableRow key={product.id}>
              <TableCell>
                <Input defaultChecked={!!liveProducts.find(
                  //@ts-ignore
                  (prod)=>prod.productId === product.default_price.id
                )}
      ...
      <Button className='mt-4' disabled={isLoading} onClick={handleSaveProducts}>Save Products</Button>
    </>
  )
}
